...
enum CreatureMesh {
    ...
    static func makeSquid() -> SCNNode {
        ...
        mantle.firstMaterial?.diffuse.contents =
            UIColor(red: 0.45, green: 0.1, blue: 0.55, alpha: 1)

        mantle.firstMaterial?.emission.contents =
            UIColor(red: 0.35, green: 0.0, blue: 0.5, alpha: 1)

        let mantleNode = SCNNode(geometry: mantle)

        mantleNode.scale =
            SCNVector3(1.0, 1.15, 1.35)
        ...
        eye.firstMaterial?.emission.contents =
            UIColor(red: 0.2, green: 0.9, blue: 1.0, alpha: 1)

        let eyeNode = SCNNode(geometry: eye)

        eyeNode.position =
            SCNVector3(0, 0.06, 0.18)
        ...
        for i in 0..<tentacleCount {
            ...
            let tent = SCNCylinder(
                radius: 0.035,
                height: 0.55
            )

            tent.firstMaterial?.diffuse.contents =
                UIColor(
                    red: 0.55,
                    green: 0.15,
                    blue: 0.7,
            ...
            tent.firstMaterial?.emission.contents =
                UIColor(
                    red: 0.25,
                    green: 0.0,
                    blue: 0.4,
    ...
    /// Deep-ocean alien fish:
    /// long body, fins, bioluminescent spots, forked tail
    static func makeDeepFish() -> SCNNode {
        ...
        // Capsule height along Y;
        // rotate so length is along Z.
        bodyNode.eulerAngles =
            SCNVector3(
                Double.pi / 2,
                0,
                0
        ...
        let lureStem = SCNCylinder(
            radius: 0.015,
            height: 0.2
        )

        lureStem.firstMaterial?.diffuse.contents =
            UIColor(
                red: 0.1,
                green: 0.4,
                blue: 0.5,
        ...
        lure.firstMaterial?.diffuse.contents =
            UIColor(
                red: 0.3,
                green: 1.0,
                blue: 0.7,
        ...
        lure.firstMaterial?.emission.contents =
            UIColor(
                red: 0.2,
                green: 1.0,
                blue: 0.6,
        ...
        for side: Float in [-1, 1] {
            ...
            eye.firstMaterial?.diffuse.contents =
                eyeMat
        ...
        for side: Float in [-1, 1] {
            ...
            fin.firstMaterial?.diffuse.contents =
                UIColor(
                    red: 0.1,
                    green: 0.45,
                    blue: 0.55,
            ...
            fin.firstMaterial?.emission.contents =
                UIColor(
                    red: 0.0,
                    green: 0.25,
                    blue: 0.35,
        ...
        for side: Float in [-1, 1] {
            ...
            tail.firstMaterial?.diffuse.contents =
                UIColor(
                    red: 0.1,
                    green: 0.5,
                    blue: 0.55,
        ...
        for i in 0..<4 {
            ...
            spot.firstMaterial?.emission.contents =
                UIColor(
                    red: 0.2,
                    green: 0.9,
                    blue: 1.0,
            ...
            spot.firstMaterial?.diffuse.contents =
                UIColor.cyan
    ...
    // MARK: - Squid Animation

    static func animateSquid(
        _ node: SCNNode,
        phase: Float
    ) {
...
enum TubePhysics {
    ...
    static func dampAgainstWall(
        ...
        if radialOffset >= maxOffset - 0.02 &&
            radialVel > 0 {
...
private protocol IdentifiableWrapper {
    ...
    var wrapperID: ObjectIdentifier {
        get
    }
}
